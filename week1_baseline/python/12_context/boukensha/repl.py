...
class Repl:
    ...
    def banner(self):
        key_status = "x API key not set"
        if self.api_key and self.api_key.strip():
            key_status = "v API key set"
        ...
        if config_exists:
            ...
            config_line = f"{self.config_dir or '(default)'}  x directory not found"
        ver = self.version or "?.?.?"
...
"""
    ...
    def _mud_status_string(self):
        ...
        try:
            ...
            return f"{host}:{port}  x not reachable"

        return f"v {host}:{port}"
